 ...
 *  and the path for maximum is 7->10.
	 ...
	 8     -4   
Solution
1) First find the leaf node that is on the maximum sum path. In the following code getTargetLeaf() does this
 	by assigning the result to target_leaf_ref.
...
class MaximumSumLeafToRootPathInABinaryTree
{
 Node root;
                 ...
                 int curr_sum)
 {
     ...
     // Update current sum to hold sum of traversed nodes on path from root to this node
 ...
 {
	 MaximumSumLeafToRootPathInABinaryTree tree = new MaximumSumLeafToRootPathInABinaryTree();
